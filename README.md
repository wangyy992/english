# 知更 Robin — AI 英语家教(原名「四技」)

单人使用的英语学习 PWA:听力精听 + 跟读、划词阅读、AI 翻译/写作批改、带 SRS 的全局生词本。核心设计原则:把每次学习的启动摩擦降到最低——打开即今日任务,一切默认预选,零确认弹窗。

## 功能一览

- **今日 + 时长规划**:选好今天学多久(记住上次选择),按权重和能力弱项把时间分到 听/说/读/写/词汇 五张任务卡,内建计时、混合完成验证、昨日欠账提示;完成 ≥70% 才算打卡。
- **听力**:盲听 → 精听(逐句 A-B 循环 + 划词查词)→ 跟读(发音评分)→ 复述(录音转写 + AI 按 rubric 批改)。
- **说**:跟读(Azure 音素级评分,无 Key 降级浏览器基础模式)、AI 情景对话(点餐/面试/机场/自定义)、雅思口语模拟(Part 1/2/3 仿真题 + 四维 band 评分)。
- **阅读**:预置文章库 + 粘贴任意英文即读,正文自动高亮已收录的生词。
- **写作**:5 句中译英逐句批改,或 AI 生成主题的自由写作,批改结果含错误定位、地道表达、结构建议。
- **生词本**:FSRS 间隔重复,Anki 式四键(重来/困难/良好/简单)+ 滑动手势,每日新卡上限可调。
- **家教大脑**:五维能力模型(EWMA 随每次活动更新)+ 雷达图、可跳过的分级测试、每日 AI 点评、积分成就与 温和/硬核 奖惩。
- **划词查词**:任意位置选中文字,秒出音标 + 英英释义,一键加入生词本,零确认弹窗。
- **PWA**:可添加到手机主屏幕,静态资源离线可用。

## 浏览器支持与已知限制

保证支持:Chrome/Edge 桌面 + Android Chrome。iOS Safari 尽力而为,已知限制:

- 浏览器语音识别(SpeechRecognition)在 iOS Safari 不可用或不稳定 → 跟读/复述/对话的"基础模式"不可用,配置 Azure Key 后可用;
- 后台/锁屏时计时暂停依赖 Page Visibility,iOS PWA 偶有延迟;
- 本地通知(硬核模式提醒)iOS PWA 不支持;
- speechSynthesis 朗读的音色与打断行为因系统而异。
